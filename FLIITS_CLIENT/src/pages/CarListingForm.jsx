...
const CarListingForm = () => {
    ...
    if (step < 4) {
      ...
      try {
        ...
        formDataToSend.append(
          "availabilityDays",
          formDataToStore.availabilityDays
        );
        formDataToSend.append(
          "availabilityHours",
          formDataToStore.availabilityHours
        );
        ...
        formDataToSend.append(
          "securityDeposit",
          formDataToStore.securityDeposit
        );
        formDataToSend.append(
          "extraMileageFee",
          formDataToStore.extraMileageFee
        );
        ...
        formDataToSend.append(
          "renterConditions",
          formDataToStore.renterConditions
        );
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SERVER_BASE_URL}/api/add-car`,
          {
            method: "POST",
            body: formDataToSend,
          }
  ...
  const renderCarInfo = () => (
    <>
      ...
      <p className="description">
        Provide information about your car to help us match it with renters
      </p>
  ...
  const renderPricingInfo = () => (
    <>
      ...
      <p className="description">
        Set your car’s rental price and additional charges.
      </p>
      ...
      <div className="photo-buttons">
        {[
          ...
          <button
            ...
            onClick={() =>
              document.getElementById(view.replace(/\s+/g, "")).click()
            }
          >
            {view}
            <input
              ...
              onChange={(e) =>
                handleFileChange(
                  e,
                  view.replace(/\s+/g, "").charAt(0).toLowerCase() +
                    view.replace(/\s+/g, "").slice(1)
                )
              }
              style={{ display: "none" }}
            />
